...
* update the lexer rules (`Lexer/lexer.l`),
* update the parser productions (`Parser/parser.y`),
* update the AST (`Ast/ast.h`, `Ast/ast.c`),
* update Z3 translation (`Z3/z3_helpers.c`),
* add tests under `tests/`.

Please include a short description of your change and any new tests.

---

## License

If you plan to publish this repository, include a `LICENSE` file (for example, MIT). Example license header:
...
```bash
make all     # if makefile present
./myparser < tests/correct/sum_odd.t
```

Generate lexer & parser manually:
